...
export function toGuidelimeStep(classicWowStep: Array<string>): GuidelimeStep {
    ...
    const toTitleCaseNoDash = (str: string): string => str.toLowerCase()
        .split('-')
    ...
    return `${actionLine} ${getQuestLine()} ${getAmount()} ${getNpc()} ${getObjective()} ${hearth()} ${coordsLine()} ${xpLine} ${getNotes()}`.replace(/\s+/g, " ").trim();
}
